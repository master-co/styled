<script setup lang="ts">
    ...
    const ExtendCustomButton = styled(CustomButton)`bg:${({ $color }) => $color}-54:hover`
    const ExtendCustomAnchor = styled.a(CustomButton)``

    // Default props
    const ButtonWithDefaultProps = styled<VNode, { color: string }>(h('button', { color: 'red', onclick: () => console.log('Click') }))`bg:${({ color }) => color}`

    // Prop composition
    const PropCompositionButton = styled<VNode, { intent: string, size: string }>(h('button', { intent: 'primary' }))`
...
<template>
    ...
    <CustomButton id="custom" $color="red">Custom</CustomButton>
    ...
    <ExtendCustomButton id="extend-custom" $color="blue">Extend</ExtendCustomButton>
    <ExtendCustomAnchor id="extend-custom-a" $color="purple">Extend & Transform Tag</ExtendCustomAnchor>
    <ButtonWithDefaultProps id="default-props">Default Props</ButtonWithDefaultProps>
